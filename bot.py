...
from credits import get_credits, add_credits, deduct_credits, register_user_credits, BETA_MODE, CREDITS_ENABLED
...
def start(message):
    ...
    referral_sys.register_user(user_id, name)
    if is_new and referrer_id:
        ok, _ = referral_sys.record_referral(referrer_id, user_id, "", name)
        if ok:
            referral_sys.notify_new_referral(bot, referrer_id, name, referral_sys.REFERRAL_FUTURE_CREDITS)
    credits = get_credits(user_id)
    beta_note = "\n\n🎉 <b>Beta Mode Active!</b>\nSabhi AI features abhi free hain.\nDosto ko invite karo — future rewards kamao!" if BETA_MODE else ""
    new_user_note = ("\n\n🎉 <b>Welcome bonus: +10 Credits!</b>" if is_new else "") + beta_note
...
def handle_menu(call):
    ...
    if action == "menu_earn":
        ...
        stats = referral_sys.get_user_stats(user_id)
        rank_w = stats.get("weekly_rank")
        rank_txt = f"\n🏆 Weekly Rank: <b>#{rank_w}</b>" if rank_w else "\n📊 Abhi tak koi referral nahi — pehle bano!"
        future_cr = stats.get("future_credits", 0)
        beta_txt = (
            "\n\n🎉 <b>Beta Mode:</b> Sab features free!\n"
            "Dosto ko invite karo — future rewards kamao!\n"
            f"🎁 Future Credits: <b>{future_cr}</b> (launch pe usable)"
        ) if BETA_MODE else f"\n💰 Har referral par <b>+{REFERRAL_CREDITS} Credits</b>!"
        bot.send_message(
            call.message.chat.id,
            f"🎁 <b>Dosto ko Invite Karo!</b>\n\n"
            f"📊 Tera stats:\n"
            f"  • Lifetime referrals: <b>{stats['lifetime_referrals']}</b>\n"
            f"  • This week: <b>{stats['weekly_referrals']}</b>\n"
            f"  • This month: <b>{stats['monthly_referrals']}</b>"
            f"{rank_txt}"
            f"{beta_txt}\n\n"
...
def cmd_profile(message):
    ...
    referral_sys.register_user(user_id, name)
    stats   = referral_sys.get_user_stats(user_id)
    rewards = referral_sys.get_user_rewards(user_id)

    join_dt = time.strftime('%d %b %Y', time.localtime(stats.get("join_date", time.time())))
    rank_w  = stats.get("weekly_rank")
    rank_m  = stats.get("monthly_rank")
    rank_w_txt = f"#{rank_w}" if rank_w else "Unranked"
    rank_m_txt = f"#{rank_m}" if rank_m else "Unranked"
    ...
    if rewards:
        for key, r in rewards.items():
            ...
            badge = r.get("badge", "⭐")
            label = "Weekly Premium" if "weekly" in key else "Monthly Champion"
            reward_lines += f"\n  {badge} <b>{label}</b> — {days_left}d left"
    else:
        reward_lines = "\n  None (karo referrals, jeeto rewards!)"

    beta_block = (
        "\n\n🎉 <b>Beta Mode Active</b>\n"
        "Sabhi features free — future mein credits use honge."
    ) if BETA_MODE else ""

    bot.reply_to(
        message,
        f"👤 <b>Tera Profile</b>\n\n"
        f"🏷️ Name: <b>{html.escape(name)}</b>\n"
        f"🆔 User ID: <code>{user_id}</code>\n"
        f"📅 Joined: <b>{join_dt}</b>\n\n"
        f"📊 <b>Referrals:</b>\n"
        f"  🔗 Lifetime: <b>{stats['lifetime_referrals']}</b>\n"
        f"  📅 This Week: <b>{stats['weekly_referrals']}</b>\n"
        f"  🗓️ This Month: <b>{stats['monthly_referrals']}</b>\n\n"
        f"🏆 <b>Leaderboard Rank:</b>\n"
        f"  📅 Weekly: <b>{rank_w_txt}</b>\n"
        f"  🗓️ Monthly: <b>{rank_m_txt}</b>\n\n"
        f"💰 <b>Credits:</b>\n"
        f"  🎁 Future Credits: <b>{stats['future_credits']}</b> (usable after launch)\n\n"
        f"🎁 <b>Active Rewards:</b>{reward_lines}"
        f"{beta_block}",
        parse_mode="HTML"
    )
...
def cmd_leaderboard(message):
    parts = message.text.strip().split()
    period = "weekly"
    if len(parts) > 1 and parts[1].lower() in ("monthly", "lifetime", "weekly"):
        period = parts[1].lower()

    board  = referral_sys.get_leaderboard(period, 10)
    user_id = message.from_user.id
    rank_mine = referral_sys.get_user_rank(user_id, period)
    days_left, hours_left = referral_sys.get_days_until_reset(period)

    period_emoji = {"weekly": "📅", "monthly": "🗓️", "lifetime": "🏆"}.get(period, "📅")
    period_label = period.capitalize()

    if not board:
        board_txt = "<i>Abhi tak koi referrals nahi. Pehle bano!</i>"
    else:
        lines = []
        for e in board:
            badge   = e["badge"]
            premium = " ⭐" if e.get("premium") else ""
            lines.append(f"{badge} <b>{html.escape(e['name'])}</b> — {e['count']} referrals{premium}")
        board_txt = "\n".join(lines)

    rank_txt = f"\n\n👤 <b>Tera Rank:</b> #{rank_mine}" if rank_mine else "\n\n👤 <b>Tera Rank:</b> Unranked — invite karo!"
    reset_txt = f"\n⏳ Next reset: <b>{days_left}d {hours_left}h</b>" if period == "weekly" else f"\n⏳ Month end mein reset."

    bot.reply_to(
        message,
        f"{period_emoji} <b>{period_label} Leaderboard — Top 10</b>\n\n"
        f"{board_txt}"
        f"{rank_txt}"
        f"{reset_txt}\n\n"
        f"📋 <i>Commands: /leaderboard weekly | monthly | lifetime</i>",
        parse_mode="HTML"
    )
...
# =========================
@bot.message_handler(commands=["admin_ref"])
def cmd_admin_ref(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    bot.reply_to(
        message,
        "🛠️ <b>Admin Referral Panel</b>\n\n"
        "Available commands:\n"
        "/admin_ref_stats — full stats\n"
        "/admin_ref_reset_weekly — reset weekly now\n"
        "/admin_ref_reset_monthly — reset monthly now\n"
        "/admin_ref_ban [user_id] — ban fake referrer\n"
        "/admin_ref_unban [user_id] — unban user\n"
        "/admin_ref_reward [user_id] [days] — give manual reward\n"
        "/admin_ref_remove [user_id] — remove reward",
        parse_mode="HTML"
    )


@bot.message_handler(commands=["admin_ref_stats"])
def cmd_admin_ref_stats(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    s = referral_sys.admin_get_stats()
    weekly_top = "\n".join(
        f"  #{i+1} uid={uid} — {cnt}"
        for i, (uid, cnt) in enumerate(s["weekly_board"])
    ) or "  (none)"
    monthly_top = "\n".join(
        f"  #{i+1} uid={uid} — {cnt}"
        for i, (uid, cnt) in enumerate(s["monthly_board"])
    ) or "  (none)"
    bot.reply_to(
        message,
        ...
        f"🎉 Beta Mode: <b>{'ON' if s['beta_mode'] else 'OFF'}</b>\n"
        f"💳 Credits Enabled: <b>{'YES' if s['credits_enabled'] else 'NO'}</b>\n\n"
        f"📅 Weekly Top 5:\n{weekly_top}\n\n"
        f"🗓️ Monthly Top 5:\n{monthly_top}",
        parse_mode="HTML"
    )
...
@bot.message_handler(commands=["admin_ref_reset_weekly"])
def cmd_admin_reset_weekly(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    referral_sys.admin_reset_weekly(bot)
    bot.reply_to(message, "✅ Weekly leaderboard reset ho gaya aur rewards assign ho gaye!")


@bot.message_handler(commands=["admin_ref_reset_monthly"])
def cmd_admin_reset_monthly(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    referral_sys.admin_reset_monthly(bot)
    bot.reply_to(message, "✅ Monthly leaderboard reset ho gaya aur champion reward assign ho gaya!")


@bot.message_handler(commands=["admin_ref_ban"])
def cmd_admin_ban(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    parts = message.text.strip().split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /admin_ref_ban [user_id]")
        return
    referral_sys.admin_ban_user(parts[1])
    bot.reply_to(message, f"✅ User {parts[1]} banned from referral system.")


@bot.message_handler(commands=["admin_ref_unban"])
def cmd_admin_unban(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    parts = message.text.strip().split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /admin_ref_unban [user_id]")
        return
    referral_sys.admin_unban_user(parts[1])
    bot.reply_to(message, f"✅ User {parts[1]} unbanned.")


@bot.message_handler(commands=["admin_ref_reward"])
def cmd_admin_add_reward(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    parts = message.text.strip().split()
    if len(parts) < 3:
        bot.reply_to(message, "Usage: /admin_ref_reward [user_id] [days]")
        return
    try:
        uid  = parts[1]
        days = int(parts[2])
        referral_sys.admin_add_reward(uid, "manual_reward", days)
        bot.reply_to(message, f"✅ {days}-day reward given to user {uid}.")
    except ValueError:
        bot.reply_to(message, "❌ Invalid days value.")


@bot.message_handler(commands=["admin_ref_remove"])
def cmd_admin_remove_reward(message):
    if str(message.from_user.id) != str(ADMIN_USER_ID):
        return
    parts = message.text.strip().split()
    if len(parts) < 2:
        bot.reply_to(message, "Usage: /admin_ref_remove [user_id]")
        return
    referral_sys.admin_remove_reward(parts[1], "manual_reward")
    bot.reply_to(message, f"✅ Manual reward removed from user {parts[1]}.")
...
if __name__ == "__main__":
    ...
    # Start referral/rewards/leaderboard scheduler
    referral_sys.start_scheduler(bot)
