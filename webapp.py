...
def api_referral_info():
    ...
    return jsonify({
        ...
        'referral_count':       stats['lifetime_referrals'],
        'weekly_referrals':     stats['weekly_referrals'],
        'monthly_referrals':    stats['monthly_referrals'],
        'lifetime_referrals':   stats['lifetime_referrals'],
        'future_credits':       stats['future_credits'],
...
def api_profile():
    ...
    reward_list = []
    for key, r in (rewards or {}).items():
        exp  = r.get('expires_at', 0)
        days = max(0, int((exp - __import__('time').time()) // 86400)) if exp else 0
        reward_list.append({
            'key':   key,
            'badge': r.get('badge', '⭐'),
            'label': 'Weekly Premium' if 'weekly' in key else 'Monthly Champion' if 'monthly' in key else 'Reward',
            'days_left': days,
        })
    return jsonify({
        'ok':      True,
...
def api_leaderboard():
    ...
    rank_mine = referral_sys.get_user_rank(user_id, period) if user_id else None
